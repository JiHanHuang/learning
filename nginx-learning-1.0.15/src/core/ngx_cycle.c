...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    //MORE:为何core_module要进行特殊处理？为何不能像其他模块一样进行解耦？
    //即使要特殊处理，可否单另函数分割，更为易读？新版本的nginx是否有改进？
    //赋值相应的配置变量给core_module,初始化core_module的conf
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_CORE_MODULE) {
            continue;
        }
    ...
    //MORE: share memory 是不是用于nginx进程之间进行数据共享的？
    ...
    //MORE:listening 数据结构的来源？ 这个if……else的作用
    ...
    //调用其他扩展模块的init_module，赋值对应的cycle
    ...
    //MORE:这里使用old_cycle和cycle有什么用？
    //MORE：以及下面的释放一些不必要的内存，fd，sock？？为何不必要？
