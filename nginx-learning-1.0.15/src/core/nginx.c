...
//ngx_cdecl 为了兼容不同平台，约定调用方式。参考：https://blog.csdn.net/midion9/article/details/50605337
int ngx_cdecl
main(int argc, char *const *argv)
{
    ngx_int_t         i;
    ngx_log_t        *log;//用于日志记录
    ngx_cycle_t      *cycle, init_cycle;//主要数据结构，相关的结构都在里面
    ngx_core_conf_t  *ccf;//系统配置变量，从配置文件里读取相应变量并转化。

#if (NGX_FREEBSD)
    ngx_debug_init();
#endif

    // 初始化了一个自己的errnum 列表，使用地方？TODO
    if (ngx_strerror_init() != NGX_OK) {
        return 1;
    }

    //参数分析，nginx主要依赖配置文件，因此命令行参数很简单。
    ...
    //MORE：看起来像获取NGINX系统变量，然后干些事情， 和主流程似乎不影响。
    ...
    //注册信号量的回调函数，信号量和回调函数的关联关系在signals全局量中。
    //在master进程的主for循环中会真正受到信号量的影响(标记位传递变化)，产生相应动作。
